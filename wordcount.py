...
cat command works on linux to show the content of a file in terminal (linux)
...
def calculate_frequencies(file_contents):
    ...
    filereader  = file_contents.lower().split()
    word={}
    word = collections.Counter(filereader)
    word={}
    lst = []
    str1="".join(ch for ch in word if ch.isalnum())
    for x in word:
        x="".join(ch for ch in x if ch.isalnum())
        if x in uninteresting_words:
            lst.append(x)
    for x in lst:
        del word[x]
